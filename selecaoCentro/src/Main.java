...
public class Main {
    public static void main(String[] args) {
        ...
        while(k<10) {
            ...
            centros = greedyAlgorithm(pontos, k, distancias);
            tempoFinal = System.currentTimeMillis();
    ...
    public static ArrayList<Ponto> greedyAlgorithm(ArrayList<Ponto> pontos, int k, ArrayList<Distancias> distancias){
        if(k == pontos.size()){
            return pontos;
        }
        ...
        int n = pontos.size();
        Random random = new Random();
        int primeiroCentro = random.nextInt(n);
